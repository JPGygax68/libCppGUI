...
namespace cppgui {
    ...
    auto Single_element_layout::compute_minimal_size() -> Extents
    {
        return { 
    ...
    */
    ...
    void Single_element_layout::compute_layout(const Extents &extents) // , const Padding &padding)
    {
        ...
        {
            text_origin->x = 0; // static_cast<Position>(padding[3]);
        }
        else if (minor_alignment == Alignment::cultural_minor_middle)
        {
            text_origin->x = (Position) (/* padding[3] + */ (w - bounding_box.width()) / 2);
        }
        else if (minor_alignment == Alignment::cultural_minor_end)
        {
            text_origin->x = (Position) (/* padding[3] + */ w - bounding_box.width());
        }
